    ...
    def __call__(
        ...
        **kwargs,
        ...
        pose_cond_tensor = self.cond_image_processor.preprocess(
            tgt_pose_image, height=height, width=width
        )
        pose_cond_tensor = pose_cond_tensor.to(
            dtype=self.vae.dtype, device=self.vae.device
        )
        pose_tgt_latents = self.vae.encode(
            pose_cond_tensor
        ...
        face_cond_tensor = self.cond_image_processor.preprocess(
            tgt_face_image, height=height, width=width
        )
        face_cond_tensor = face_cond_tensor.to(
            dtype=self.vae.dtype, device=self.vae.device
        )
        face_tgt_latents = self.vae.encode(
            face_cond_tensor
        ).latent_dist.mean  # (bs, d, 64, 64)
        face_tgt_latents = face_tgt_latents * 0.18215

        hand_cond_tensor = self.cond_image_processor.preprocess(
            tgt_hand_image, height=height, width=width
        )
        hand_cond_tensor = hand_cond_tensor.to(
            dtype=self.vae.dtype, device=self.vae.device
        )
        hand_tgt_latents = self.vae.encode(
            hand_cond_tensor
        ).latent_dist.mean  # (bs, d, 64, 64)
