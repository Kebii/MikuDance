...
@dataclass
class MikuDanceVideoPipelineOutput(BaseOutput):
    videos: Union[torch.Tensor, np.ndarray]


class MikuDanceVideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        tracker_ts = torch.from_numpy(scene_motion_npy).to(
            dtype=self.vae.dtype, device=self.vae.device
        )
        tracker_ts = tracker_ts[None,].repeat((bs, 1, 1, 1, 1))
        tracker_ts = rearrange(tracker_ts, "b f d w h -> (b f) d w h")
        ...
        if self.video_decoder:
            images = self.decode_temporal(latents)
        else:
        ...
        return MikuDanceVideoPipelineOutput(videos=images)
